...
import updateAvatarColorService from "../services/user/updateAvatarColor.service";
...
const updateAvatarColorController = async (req: Request, res: Response) => {
  try {
    const userId = req.user.id;
    const { color } = req.body;
    const user = await updateAvatarColorService({ userId, color });
